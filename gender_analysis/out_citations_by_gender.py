...
if __name__ == "__main__":
    ...
    out_citation_by_year = defaultdict(lambda: defaultdict(lambda: 0))


    for paper in tqdm(lazy_paper_reader(inp_fn)):
        cur_year_record = out_citation_by_year[paper["year"]]

        # all authors stats
        for cur_author in paper["authors"]:
            cur_year_record[cur_author["gender"]] += len(paper["outCitations"])

    logging.info("Writing to {}".format(out_fn))

    header = ["year", "male-out-citations", "female-out-citations", "unknown-out-citations"]
    with open(out_fn, 'w') as fout:
        fout.write("{}\n{}".format(','.join(header),
                                   "\n".join([','.join(map(str,
                                              ...
                                              in sorted(dict(out_citation_by_year).iteritems())])))
